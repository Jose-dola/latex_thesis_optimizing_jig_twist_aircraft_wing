...
fittype = fittype('a0 + a1*cos(x) + a2*sin(x)');
p = fit(xt',f(xt)',fittype,'StartPoint',[1,1,1])
...
plot(x,p(x),'DisplayName','cos/sin regression: f(x) = a0 + a1*cos(x) + a2*sin(x)','Color',regressionmodellinecolor,'LineWidth',linewidth);
...
Y = ry'*ones(1,10);
Y = Y';
Z = p.a0 + p.a1*X + p.a2*Y;

xGraph = @(x) cos(x);
yGraph = @(x) sin(x);

figure()
surf(X,Y,Z,'FaceColor','red','EdgeColor','none','FaceAlpha',0.3,'DisplayName','Plane: f(Phi1,Phi2) = a0 + a1*Phi1 + a2*Phi2');
hold on;
plot3(xGraph(x),yGraph(x),f(x),'DisplayName','line: f(x) = 3*x^2+2*x+1','Color',functionlinecolor,'LineWidth',linewidth);
plot3(xGraph(x),yGraph(x),p(x),'DisplayName','line: f(x) = a0 + a1*cos(x) + a2*sin(x)','Color',regressionmodellinecolor,'LineWidth',linewidth);
scatter3(xGraph(xt),yGraph(xt),f(xt),pointssize,pointtype,pointscolor,'DisplayName','training points');
xlabel('Phi1 / cos(x)','FontName',fname,'fontsize',fsize);
ylabel('Phi2 / sin(x)','FontName',fname,'fontsize',fsize);
zlabel('f(Phi1,Phi2) / f(x)','FontName',fname,'fontsize',fsize);
ha=gca;set(ha,'linewidth',linewidth,'FontName',fname,'FontSize',fsize,'Box','off');
legend show;
hold off;
