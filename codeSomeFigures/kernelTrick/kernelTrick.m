...
figure()
plot(x,f(x),'DisplayName','function: f(x) = 3*x^2+2*x+1','Color',functionlinecolor,'LineWidth',linewidth);
hold on;
scatter(xt,f(xt),pointssize,pointtype,pointscolor,'DisplayName','training points');
plot(x,p(x),'DisplayName','linear regression','Color',regresionlinecolor,'LineWidth',linewidth);
...
figure()
surf(X,Y,Z,'FaceColor','red','EdgeColor','none','FaceAlpha',0.3,'DisplayName','plane: f(Phi1,Phi2) = 3*Phi1+2*Phi2+1');
hold on;
plot3(x.*x,x,f(x),'DisplayName','line: f(x) = 3*x^2+2*x+1','Color','b','LineWidth',linewidth);
scatter3(xt.*xt,xt,f(xt),pointssize,pointtype,pointscolor,'DisplayName','training points');
xlabel('Phi1 / x^2','FontName',fname,'fontsize',fsize);
ylabel('Phi2 / x','FontName',fname,'fontsize',fsize);
