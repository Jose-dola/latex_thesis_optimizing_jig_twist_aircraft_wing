function a = getGaussianPriorAndPosteriorSamplesUniDimensional(f,x_training,n,x_test,nt,y_training,prior,numberofsamplefunctions,s,l,sdnoise) 
...
if prior == 1
 ...
 for i = 1:numberofsamplefunctions;
   y_prior = mvnrnd(zeros(n+nt,1),K);
   xy = [xxt.' y_prior.'];
   xysort = sortrows(xy,1);
   C = c( mod(i+1,length(c)) , :);
   s = scatter(xysort(:,1),xysort(:,2),pointssize*3,pointtype);
   s.MarkerEdgeColor = C;
   s.MarkerFaceColor = C;
   plot(xysort(:,1),xysort(:,2),'Color',C,'linewidth',3);
 end
 title_str = sprintf('3 samples from Gaussian prior');%,sdnoise);
 t = title(title_str);
 %t.FontWeight = 'bold';
 %t.FontSize = 60;
 ...
 hold off;
 str = sprintf('priorPosterior/3SAMPLESPrior.fig');%,sdnoise);
 savefig(str);
 str = sprintf('priorPosterior/3SAMPLESPrior.eps');%,sdnoise);
 saveas(gcf,str,'epsc');
end
...
ha=gca;set(ha,'linewidth',linewidth,'FontName',fname,'FontSize',fsize,'Box','off');
hold off;
str = sprintf('priorPosterior/3SAMPLESPosterior.fig');%,sdnoise);
savefig(str);
str = sprintf('priorPosterior/3SAMPLESPosterior.eps');%,sdnoise);
saveas(gcf,str,'epsc');
